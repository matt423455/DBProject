...
// Check if RSO exists and is pending (status = 2).
$stmt = $conn->prepare("SELECT * FROM RSO WHERE rso_id = ? AND status = 2");
...
// Approve RSO by updating status to 1.
$stmt = $conn->prepare("UPDATE RSO SET status = 1 WHERE rso_id = ?");
$stmt->bind_param("i", $rso_id);
$stmt->execute();
